/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   pars3.c                                            :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: mteffahi <mteffahi@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/05/20 20:09:53 by mteffahi          #+#    #+#             */
/*   Updated: 2025/08/15 15:18:10 by mteffahi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../include/exec.h"

void cpy(char *vl, char *input, int pos, int len)
{
	int	tmp;

	tmp = 0;
	while (tmp < len)
	{
		vl[tmp] = input[pos + tmp];
		tmp++;
	}
	vl[tmp] = '\0';
}

// static char	*two_qt(t_ptr **head, char *input, int *i)
// {
// 	if (ft_strlen(input) == 2)
// 	{
// 		if (input[*i] == '\"' && input[(*i) + 1] == '\"')
// 		{
// 			(*i) += ft_strlen(input);
// 			return (ft_strdup1(head, input));
// 		}
// 		else if (input[*i] == '\'' && input[(*i) + 1] == '\'')
// 		{
// 			(*i) += ft_strlen(input);
// 			return (ft_strdup1(head, input));
// 		}
// 	}
// 	return (NULL);
// }

// int	splt_quoted(t_ptr **head, t_tkn **token, char *input, int *i)
// {
// 	int			len;
// 	char		*vl;
// 	int			pos;
// 	char		q_type;

// 	if (!input)
// 		return (ft_mall(head, 0), 0);
// 	if (input[*i] == '\"' && input[(*i) + 1] == '\"')
// 	{
// 		vl = two_qt(head, input, i);
// 		creat_tkn_node(head, token, vl, identify_tkn(vl));
// 		return (0);
// 	}
// 	if (input[*i] == '\'' && input[(*i) + 1] == '\'')
// 	{
// 		vl = two_qt(head, input, i);
// 		creat_tkn_node(head, token, vl, identify_tkn(vl));
// 		return (0);
// 	}
// 	q_type = input[*i];
// 	(*i)++;
// 	len = 0;
// 	pos = *i;
// 	while (input[*i] && input[*i] != q_type)
// 	{
// 		(*i)++;
// 		len++;
// 	}
// 	if (input[*i] == q_type)
// 		(*i)++;  // Skip closing quote
// 	if (len == 0 && (*i) >= 3 && input[(*i) - 3] != '=')
// 	{
// 		if (input[*i])
// 			(*i)++;
// 		return (0);
// 	}
// 	vl = ft_mall(head, (len + 1));
// 	cpy(vl, input, pos, len);
// 	creat_tkn_node(head, token, vl, identify_tkn(vl));
// 	return (0);
// }

int	splt_quoted(t_ptr **head, t_tkn **token, char *input, int *i)
{
	int		len;
	char	*vl;
	int		pos;
	char	q_type;

	if (!input)
		return (ft_mall(head, 0), 0);

	q_type = input[*i];  // store quote type
	(*i)++;              // skip opening quote
	pos = *i;
	len = 0;

	// Handle empty quotes right away: "" or ''
	if (input[*i] == q_type)
	{
		(*i)++; // skip closing quote
		vl = ft_mall(head, 1);
		vl[0] = '\0';
		creat_tkn_node(head, token, vl, identify_tkn(vl));
		return (0);
	}

	// Scan until we find the matching quote or end of string
	while (input[*i] && input[*i] != q_type)
	{
		(*i)++;
		len++;
	}

	// If we found a closing quote, skip it
	if (input[*i] == q_type)
		(*i)++;

	// Even if len == 0, we have already moved *i — no infinite loop

	vl = ft_mall(head, len + 1);
	cpy(vl, input, pos, len);
	creat_tkn_node(head, token, vl, identify_tkn(vl));

	return (0);
}


char	*gt_nm(char *input, int *i, t_ptr **head)
{
	int		pos;
	int		len;
	char	*v_name;

	pos = *i;
	len = 0;
	while (input[pos] && ft_isalnum(input[pos]))
	{
		pos++;
		len++;
	}
	v_name = ft_mall(head, (len + 1));
	pos = 0;
	while (input[*i] && ft_isalnum(input[*i]) && pos < len)
	{
		v_name[pos] = input[*i];
		pos++;
		(*i)++;
	}
	v_name[len] = '\0';
	return (v_name);
}

int	expand_var(char *input, int *i, t_tkn **tkn_head, t_ptr **head_ptr)
{
	const char *env;
	const char	*nm;

	(*i)++;
	nm = gt_nm(input, i, head_ptr);
	env = ft_strdup1(head_ptr, getenv(nm));
	if (!env)
	{
		(*i)++;
		ft_mall(head_ptr, 0);
		return (ft_putstr_fd("", 1), 0);
	}
	creat_tkn_node(head_ptr, tkn_head, (char *)env, identify_tkn(((char *)env)));
	// (*i)++;
	return (1);
}
