...
/*   Updated: 2025/08/17 22:40:56 by ilallali         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				execute_external_command(const char *exec_path, t_cmd *command,
					char **original_envp);
char			*resolve_command_path(const char *command_name,
					t_env_copy *env_list);
void			child_process_execution(const char *exec_path, char **args,
					char **original_envp);
...
int				execute_builtin_command(t_builtin_id id, t_cmd *command,
					t_shell *shell);
...
int				set_env_value(t_env_copy **env_list, const char *key,
...
void			creat_tkn_node(t_ptr **head, t_tkn **tkn_head, char *val,
					t_tkn_type type);
int				handle_rdr(t_ptr **head, t_tkn **token, char *input, int *i);
int				expand_var(char *input, int *i, t_tkn **tkn_head,
					t_ptr **head_ptr);
...
char			*extract_vl(t_ptr **ptr_head, t_env_copy **env_head,
					char *var_name);
int				last_pipe_check(t_tkn **head);
void			get_shell_pid_str(t_ptr **head, int *pos);
int				tokenize_input(t_shell *shell, t_ptr **head, char *input,
					t_tkn **tkn_head);
...
void			handle_input_rdr(t_ptr **head, t_tkn **token, char *input,
					int *i);
void			handle_output_rdr(t_ptr **head, t_tkn **token, char *input,
					int *i);
int				calculate_token_length(char *input, int *i);
char			*extract_token_value(t_ptr **head, char *input, int pos,
					int len);
